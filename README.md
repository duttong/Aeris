...
<pre><code><p>1. df2 = df.groupby(['seq_count','ssv'], as_index=False).apply(lambda x: x.iloc[x.ssv.size//2:]).reset_index(level=0, drop=True)</code></pre>
<pre><code><p>2. df2.groupby(['seq_count','ssv']).mean() or .std()</code></pre>
